...
/**
 * Dress the composer card with the shared corner ornament: a transparent
 * container holding four corner spans (the global corner-position rules
 * place and mirror each one), plus the gold frame lines that connect them.
 */
function decorateComposerCorners(ownedNodes: Set<Element>): void {
  const card = document.querySelector<HTMLElement>('[data-composer-card]')
  if (card === null || card.querySelector("[data-skin-chrome='composer-corners']") !== null) return
  const corners = document.createElement('div')
  corners.dataset.skinChrome = 'composer-corners'
  corners.dataset.skinOwner = SKIN_OWNER
  corners.setAttribute('aria-hidden', 'true')
  for (const position of ['top-left', 'top-right', 'bottom-right', 'bottom-left']) {
    const corner = document.createElement('span')
...
export function apply(ctx: Context): void {
  ...
  decorateComposerCorners(ownedNodes)
  syncProjectedState()
  ...
  // 白天/夜晚模式切换开关：直接切换应用主题属性 data-ds-dark-theme
  const nightToggle = document.createElement('button')
  nightToggle.dataset.skinChrome = 'night-toggle'
  nightToggle.dataset.skinOwner = SKIN_OWNER
  nightToggle.setAttribute('aria-label', '切换白天/夜晚模式')
  nightToggle.title = '白天 / 夜晚模式'
  Object.assign(nightToggle.style, {
    position: 'fixed',
    right: '18px',
    bottom: '74px',
    zIndex: '60',
  ...
  const syncNightToggle = (): void => {
    nightToggle.textContent = document.body.hasAttribute('data-ds-dark-theme') ? '☀️' : '🌙'
  }
  const setNight = (night: boolean): void => {
    document.body.toggleAttribute('data-ds-dark-theme', night)
    syncNightToggle()
    try {
      localStorage.setItem('maid-atelier-night', night ? '1' : '0')
    } catch {
      // 忽略存储不可用
    }
  }
  nightToggle.addEventListener('click', () => {
    setNight(!document.body.hasAttribute('data-ds-dark-theme'))
  })
  syncNightToggle()
  try {
    const storedNight = localStorage.getItem('maid-atelier-night')
    if (storedNight === '1' || storedNight === '0') setNight(storedNight === '1')
  } catch {
    // 忽略存储不可用
  }
  ownedNodes.add(nightToggle)
  document.body.append(nightToggle)
}
